...
public class CustomReplyingKafkaTemplate<K, V extends KMessage<?>, R extends KMessage<?>> extends KafkaTemplate<K, V>
		implements BatchMessageListener<K, R>, InitializingBean, SmartLifecycle, DisposableBean,
	...
	public CustomReplyingKafkaTemplate(ProducerFactory<K, V> producerFactory,
	...
	public RequestReplyFuture<K, V, R> sendAndReceive(ProducerRecord<K, V> record) {
		...
		record.value().setCorrelationId(correlationId.getCorrelationId());
	...
	public void onMessage(List<ConsumerRecord<K, R>> data) {
		data.forEach(record -> {
			...
			if (correlationId == null && record.value().getCorrelationId() != null) {
				System.out.println("**** got correlationId from KMessage");
				correlationId = new CorrelationKey(record.value().getCorrelationId());
			}
			if (correlationId == null) {
				...
				if (future == null) {
					this.logger.error("No pending reply: " + record + " with correlationId: " + correlationId
							+ ", perhaps timed out");
				} else {
					if (this.logger.isDebugEnabled()) {
